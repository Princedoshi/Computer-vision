...
class ImageSelector(object):
    ...
    def __prepare_cluster_sets__hdbscan(self, files):
        ...
        if not files_clusters_index_array_of_only_one_image:
            files_clusters_index_array_of_only_one_image.append(np.array([]))
    ...
    # def select_best_frames(self, input_key_frames, number_of_frames):
        ...
        if len(input_key_frames) >= 1:
            ...
            if not files_clusters_index_array_of_only_one_image:
                  files_clusters_index_array_of_only_one_image = [np.array([])]
